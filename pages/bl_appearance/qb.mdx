...
<Steps>
### 1. Modify `qb-multicharacter:server:getSkin` Callback
...
- Go to the callback definition around `line 183` in `server/main.lua` and change it to:
    ```lua
...
### 2. Modify `cDataPed` RegisterNUICallback
...
### 3. Modify `initializePedModel` around `line 34` in `client/main.lua` and replace it with the following:
    ```lua
